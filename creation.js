...
// --- Before/After pair dataset for the Spark gallery ---
// Each pair: a stock "photo" (SVG-built placeholder) + a "painting" (SVG-built)
// In production these become real photos + AI-restyled paintings.
const PAIRS = [
  { before: makePhoto('#d9b681', '#5a3018', true, 'couple'),
    after: makePainting('vangogh'), beforeName: 'A candid snap',
    sig: '— V. van Gogh' },
  { before: makePhoto('#c89a6a', '#2a1a0c', false, 'solo'),
    after: makePainting('monet'), beforeName: 'Portrait No. 02',
    sig: '— C. Monet' },
  { before: makePhoto('#e8c8a0', '#3a2614', true, 'couple'),
    after: makePainting('picasso'), beforeName: 'Wedding, 2026',
    sig: '— P. Picasso' },
  { before: makePhoto('#b2824e', '#2a1a0c', false, 'solo'),
    after: makePainting('warhol'), beforeName: 'The regular',
    sig: '— A. Warhol' },
  { before: makePhoto('#d0a480', '#4a2a1a', true, 'trio'),
    after: makePainting('hokusai'), beforeName: 'Group shot',
    sig: '— Hokusai' },
];

const PAINTER_MAP = {
  vangogh: { name: 'Van Gogh', sig: '— V. van Gogh', meta: 'vangogh' },
  monet:   { name: 'Monet',    sig: '— C. Monet',    meta: 'monet' },
  picasso: { name: 'Picasso',  sig: '— P. Picasso',  meta: 'picasso' },
  warhol:  { name: 'Warhol',   sig: '— A. Warhol',   meta: 'warhol' },
  hokusai: { name: 'Hokusai',  sig: '— Hokusai',     meta: 'hokusai' },
};
...
function makePhoto(bg, hair, smile, kind) {
  ...
  const faceSvg = centers.map((cx, i) => {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 125" preserveAspectRatio="xMidYMid slice">
    <defs>
...
function makePainting(style) {
  const palettes = {
    vangogh:  { sky: '#1e3a8a', accent: '#f4b731', skin: '#e0b878', hair: '#2a1a0c', cloth: '#8a2e14' },
    monet:    { sky: '#cde1f2', accent: '#f5c5da', skin: '#f0dcc0', hair: '#7a5a8a', cloth: '#b2d4e8' },
    picasso:  { sky: '#c9a878', accent: '#d87a3c', skin: '#e8d0a8', hair: '#2a2018', cloth: '#5a4838' },
    warhol:   { sky: '#ff2d88', accent: '#ffdc00', skin: '#f8c8d8', hair: '#00c8dc', cloth: '#2a1a0c' },
    hokusai:  { sky: '#d8e6f0', accent: '#1e5aa8', skin: '#f0dcc0', hair: '#1e3050', cloth: '#1e5aa8' },
  };
...
function onHeroScroll() {
  ...
  const p = Math.max(0, Math.min(1, -rect.top / (hero.offsetHeight - vh * 0.6)));
...
function renderPair(idx, painterKey) {
  const pair = PAIRS[idx];
  sparkBeforeImg.style.backgroundImage = pair.before;
  sparkBeforeName.textContent = pair.beforeName;

  // If a painter was explicitly selected, use that; else use the pair's default
  const keys = Object.keys(PAINTER_MAP);
  const key = painterKey || keys[idx % keys.length];
  document.body.dataset.painter = key;
  sparkAfterImg.style.backgroundImage = makePainting(key);
  ...
  // Update arrow label
  sparkArrowLabel.textContent = `repainted by Claude · ${PAINTER_MAP[key].name}`;

  // Trigger painting emerge animation
...
function syncMetaPicker(key) {
  ...
  if (afterBrand) afterBrand.textContent = 'Playing With Photo · ' + PAINTER_MAP[key].name + ' Edition';
}

function advanceGallery() {
  if (userLocked) return;
  fireFlash();
  // Wait for flash peak, swap mid-flash for dissolve effect
  setTimeout(() => {
    currentPairIndex = (currentPairIndex + 1) % PAIRS.length;
    renderPair(currentPairIndex);
  }, 350);
}
...
// Painter picker — flash + repaint + lock
if (sparkPicker) {
  sparkPicker.addEventListener('click', e => {
    ...
    fireFlash();
    setTimeout(() => {
      renderPair(currentPairIndex, btn.dataset.painter);
    }, 350);
  });
}
...
if (legacyMetaPicker) {
  legacyMetaPicker.addEventListener('click', e => {
    ...
    if (!PAINTER_MAP[key]) return; // "vintage" fallback is OK; we just sync meta
    userLocked = true;
    clearInterval(autoTimer);
    fireFlash();
    setTimeout(() => renderPair(currentPairIndex, key), 350);
  });
}

// Initialize gallery
if (sparkBeforeImg) {
  renderPair(0, 'vangogh');
  startAutoRotate();
}
  ...
  try {
    ...
    const draw = () => {
      ...
      if (sparkVideo.videoWidth) {
        ...
        ctx.save();
        ctx.filter = 'sepia(.7) saturate(1.3) contrast(1.05) brightness(.95) hue-rotate(-5deg)';
        ctx.translate(w, 0); ctx.scale(-1, 1);
        const vw = sparkVideo.videoWidth, vh = sparkVideo.videoHeight;
